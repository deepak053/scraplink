...
export function ListScrapPage() {
  ...
  const handlePredictPrice = async () => {
    if (selectedLeaves.length === 0) {
      alert("Pick at least one item and enter weights.");
      return;
    }
  ...
  /** ---------- Submit (UPDATED: provides sub_category & sub_sub_category) ---------- */
  const handleSubmit = async () => {
    ...
    // derive sensible sub_category (unique list of selected subcategories)
    const uniqueSubCategories = Array.from(
      new Set(selectedLeaves.map((s) => (s.subcategory || "").trim()).filter(Boolean))
    );
    const subCategoryValue =
      uniqueSubCategories.length > 0 ? uniqueSubCategories.join(", ") : (selectedCategories[0] || "uncategorized");

    // derive sub_sub_category from selected leaves
    const leafNames = selectedLeaves.map((s) => s.leaf).filter(Boolean);
    const subSubCategoryValue = leafNames.length > 0 ? Array.from(new Set(leafNames)).join(", ") : "uncategorized";
    ...
    try {
      const { error } = await supabase.from("scrap_listings").insert({
        ...
        sub_sub_category: subSubCategoryValue,
        description,
        weight: totalWeight,
